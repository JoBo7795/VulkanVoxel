...
VkImageView Image::CreateImageArrayView(VkImage& imageArray, VkImageView& imageViewArray, VkFormat format, VkImageAspectFlags aspectFlags, uint32_t mipLevels) {
    VkImageViewCreateInfo viewInfo{};
    viewInfo.sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO;
    viewInfo.image = imageArray;
    ...
    //VkImageView imageViewArra;
    if (vkCreateImageView(VulkanDevices::GetInstance()->GetDevice(), &viewInfo, nullptr, &imageViewArray) != VK_SUCCESS) {
        throw std::runtime_error("failed to create texture image view!");
    }
...
void Image::CreateImage(uint32_t width, uint32_t height, uint32_t mipLevels, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory, uint32_t layerCount) {
...
void Image::TransitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout, uint32_t mipLevels, uint32_t layerCount) {
...
void Image::CopyTextureDataToLayer(uint32_t width, uint32_t height, VkImage srcImage, VkImage dstImage, uint32_t layerIndex, VkCommandBuffer commandBuffer) {
